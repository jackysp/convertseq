...
func syncSeq(db *sql.DB) {
	...
	for {
		...
		_, err = trx.Exec(`REPLACE INTO test.sequence_sync (schema_name, sequence_name, create_sql) ` + sequenceQuery)
		if err != nil {
...
func restoreSeq(db *sql.DB) error {
	// Retrieve desired sequences from test.sequence_sync
	desiredSequences := make(map[string]string)
	rows, err := db.Query("SELECT CONCAT(schema_name, '.', sequence_name) AS seq_name, create_sql FROM test.sequence_sync;")
	if err != nil {
		return err
	}
	defer rows.Close()
